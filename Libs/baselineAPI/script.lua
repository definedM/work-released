...
baselineAPI = {
    ...
    --table
    ['search'] = {
        ['byName'] = function(a,b) --seems to act like Instance:FindFirstDescendant()
        ...
        ["byMaterial"] = function(a, b, c) --wtf this actualy loks ugly as hel
            pcall(function()
                local size = c
    ...
    ['childTable'] = function(path) -- looks prety useles
        local rtable = {}
        for _, pathVal in pairs(path:GetChildren()) do
            table.insert(rtable, pathVal)
        end
        return rtable
    end,
    ['findvalue'] = function(table, valueToFind) -- may create a sublibrary (to be able to use baselineAPI.dictionary.find())
        for _, tabVal in pairs(table) do
            if tabVal ~= valueToFind then continue end
            return true
        end
    end,
    ['returnvalue'] = function(table, valueToFind)
        for _, tabVal in pairs(table) do
            if string.match(valueToFind, tabVal) then
                return tabVal
            end
        end
        return false
